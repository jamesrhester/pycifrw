...
# Modified by James Hester, Dec 2000, to use re instead of
# regex.  This involves only the grammar definition for the
...
#
class CFSMachine(kjCParser.FSMachine):

  def __init__(self, nonterm):
      kjCParser.FSMachine.__init__(self, nonterm)
...
class ruleset:
  ...
  def CompFollow(self):
     ...
     kjSet.AddArc(Follow, self.StartNonterm, kjCParser.ENDOFFILETOKEN)
...
def InterpRule(list, ObjectGram):
  ...
  rule = kjCParser.ParseRule( ruleNonterm, ruleBody )
  rule.Name = ruleName
  return rule
...
def ruleGrammar():
   ...
   Root = kjCParser.nonterminal("Root")
   Rulelist = kjCParser.nonterminal("RuleList")
   Rule = kjCParser.nonterminal("Rule")
   RuleName = kjCParser.nonterminal("RuleName")
   NonTerm = kjCParser.nonterminal("NonTerm")
   Body = kjCParser.nonterminal("Body")

   # declare rules
   #   Root >> NonTerm :: Rulelist
   InitRule = kjCParser.ParseRule( Root, \
               [NonTerm, TwoColons, Rulelist], RootReduction )
   #   Rulelist >>
   RLNull = kjCParser.ParseRule( Rulelist, [], NullRuleList)
   #   Rulelist >> Rule Rulelist
   RLFull = kjCParser.ParseRule( Rulelist, [Rule,Rulelist], FullRuleList)
   #   Rule >> "@R :: NonTerm >> Body
   RuleR = kjCParser.ParseRule( Rule, \
      [RStart, RuleName, TwoColons, NonTerm, LeadsTo, Body],\
      InterpRule)
   #   Rulename >> ident
   RuleNameR = kjCParser.ParseRule( RuleName, [ident], InterpRuleName)
   #   NonTerm >> ident
   NonTermR = kjCParser.ParseRule( NonTerm, [ident], InterpNonTerm)
   #   Body >>
   BodyNull = kjCParser.ParseRule( Body, [], NullBody)
   #   Body >> ident Body
   BodyFull = kjCParser.ParseRule( Body, [ident,Body], FullBody)
   ...
   Result = kjCParser.Grammar( LexD, RuleSet.DFA, Rules, RuleDict )
   return Result
...
#
class CGrammar(kjCParser.Grammar):
   ...
   def whitespace(self,whitespacestring):
     self.LexD.whitespace(whitespacestring)
   ...
   def Nonterms(self, StringofNonterms):
     ...
     for NonTerm in nonTermlist:
        self.NonTermDict[NonTerm] = kjCParser.nonterminal(NonTerm)
...
if RUNTESTS:
  ...
  E = kjCParser.nonterminal("E")
  T = kjCParser.nonterminal("T")
  Tp = kjCParser.nonterminal("Tp")
  Ep = kjCParser.nonterminal("Ep")
  F = kjCParser.nonterminal("F")
  rule1 = kjCParser.ParseRule( E, [ T, Ep ] )
  rule2 = kjCParser.ParseRule( Ep, [ plus, T, Ep ] )
  rule3 = kjCParser.ParseRule( Ep, [ ] )
  rule4 = kjCParser.ParseRule( T, [ F, Tp ] )
  rule5 = kjCParser.ParseRule( Tp, [ star, F, Tp ] )
  rule6 = kjCParser.ParseRule( Tp, [ ] )
  rule7 = kjCParser.ParseRule( F, [ oppar, E, clpar ] )
  rule8 = kjCParser.ParseRule( F, [ id ] )
  ...
  def TESTDFA( STRING , ttt, DFA, Rulelist, DOREDUCTIONS = 1):
    ttt.STRING = STRING
    #ttt.List = kjCParser.LexList(LD0, ttt.STRING)
    ttt.Stream = kjCParser.LexStringWalker( ttt.STRING, LD0 )
    ttt.Stack = {-1:0}# Walkers.SimpleStack()
    ttt.ParseObj = kjCParser.ParserObj( Rulelist, \
  ...
  S = kjCParser.nonterminal("S")
  M = kjCParser.nonterminal("M")
  A = kjCParser.nonterminal("A")
  rr1 = kjCParser.ParseRule( S, [M] )
  #rr2 = kjCParser.ParseRule( A, [A, plus, M])
  #rr3 = kjCParser.ParseRule( A, [M], echo)
  #rr4 = kjCParser.ParseRule( M, [M, star, M])
  rr5 = kjCParser.ParseRule( M, [oppar, M, clpar])
  rr6 = kjCParser.ParseRule( M, [id])
  rl1 = [rr1,rr5,rr6]
  rs1 = ruleset(S, rl1)
  rs1.CompFirst()
  rs1.CompFollow()
  ...
  X = kjCParser.nonterminal("X")
  Y = kjCParser.nonterminal("Y")
  RX = kjCParser.ParseRule( X, [ oppar, Y, clpar ] )
  RY = kjCParser.ParseRule( Y, [] )
  rl2 = [RX,RY]
  rs2 = ruleset(X, rl2)
  rs2.CompFirst()
  rs2.CompFollow()
  ...
  S = kjCParser.nonterminal("S")
  L = kjCParser.nonterminal("L")
  R = kjCParser.nonterminal("R")
  RS1 = kjCParser.ParseRule( S, [L, equals, R] )
  RS2 = kjCParser.ParseRule( S, [R], echo )
  RL1 = kjCParser.ParseRule( L, [star, R])
  RL2 = kjCParser.ParseRule( L, [id])
  RR1 = kjCParser.ParseRule( R, [L] )
