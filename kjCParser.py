...
#
# Changes by JRH for Cif modules, 2000
# Remove use of whitespace; this spoils our
# implementation of the Cif grammar, which
# relies on it
...
#
#      LD.whitespace(reg_expression)
#         registers whitespace
#         (added by JRH, 2000)
#
...
class LexDictionary:
  ...
  def __init__(self):
    ...
    self.regexprlist = []
    self.whitespacere = WHITEREGEX
    self.whitespaceval = WHITERE
  ...
  # register a string as whitespace
  def whitespace(self,somewhitesp):
    self.whitespaceval = somewhitesp
    self.whitespacere = re.compile(self.whitespaceval)
    return
  ...
  def Token(self, String, StartPosition):
     ...
     while not finished:
        ...
        # skip whitespace
        whitespacefound = 0
        newpos = self.whitespacere.match(String, StartPosition)
        if newpos:
           StartPosition = newpos
           totalOffset = totalOffset + newpos - StartPosition
           whitespacefound = 1
...
class lexdictionary:
   ...
   def __init__(self):
       ...
       self.whitespaceval = WHITERE
       self.whitespacere = re.compile(WHITERE)
   ...
   def whitespace(self,whitespaceval):
       self.whitespaceval = whitespaceval
   ...
   def Token(self, String, StartPosition):
       ...
       while not finished:
          ...
          newmatch = skipprog.match(String, StartPosition)
          if newmatch:
             newpos = newmatch.end()
          ...
          # print 'Now looking:' + String[StartPosition:StartPosition+60]
          thismatch = termregex.match(String, StartPosition)
          if thismatch:
               g = thismatch.groupdict()
               for (term, regex, flag, fn) in self.termlist:
                 ...
                 if test:
                    ...
                    result = self.lastresult = (
                       (flag, value), thismatch.end() - StartPosition +totalOffset)
                    return result
   ...
   def compile(self):
       ...
       import re
       skipregexen = self.commentstrings + [self.whitespaceval]
