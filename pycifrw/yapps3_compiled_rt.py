...
class Scanner:
	...
	if self.scantype == "flex" and self.have_star_scan:
	    self.StarScan.prepare(input)
	    self.scan = self.compiled_scan
	    self.token = self.compiled_token
	    self.__del__ = self.StarScan.cleanup
	...
	if self.scantype != "flex":
	    ...
	    out_tokens = self.StarScan.last_ten()
	    for t in out_tokens:
